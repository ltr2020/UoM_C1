...
price = pd.Series([1000, 2000, 3000], index=[
                  "S10", "S11", "S12"])  # change index
price.name = "price"
...
frame = pd.DataFrame(
    ...
    columns=[
        ...
        "pop",
        "debt"])  # arrange column
# assign list, array, series to DataFrame
value = pd.Series([-1.5, -2, -3], index=[0, 3, 4])
frame["debt"] = value
print(frame)
...
# drop row and column, inplace=True (don't create new object alter
# original), axis=0 by default
print(frame.drop("state", inplace=False, axis=1))
# or
...
print("TwoB")
df = pd.read_csv(
    r"C:\Users\user\PycharmProjects\UoM_C1\Doc\Admission_Predict.csv",
    index_col=1)
...
print(criteria.head())
# and
criteria = df["Chance of Admit"].gt(
    0.7) & df["Chance of Admit"].lt(0.9)  # 0.7<x<0.9
# .dropna() removes entries outside 0.7< x <0.9
...
# for large file, without any missing value, add na_filter=False can
# improve perf
...
# Another example
# no change--》missing values
LOG = pd.read_csv(r"C:\Users\user\PycharmProjects\UoM_C1\Doc\log.csv")
print(LOG.head(10))
LOG = LOG.set_index(["time", "user"])  # multi-level indexing
LOG = LOG.sort_index()  # sort by index
# to fill missing values with previous row entry
...
USA["First Name"] = USA["First Name"].replace(
    to_replace="[ ].*", value="", regex=True)
del(USA["First Name"])
# method 2 apply() function apply arbitrary function written to Series
# (single col) or DF
